...
We have seen that pre and post conditions specify what must be true
...
Then we have \texttt{$y = 2\times7 = 14$}, so our postcondition is true. If 
$x$ is not 7, then we have that $y$ is true by the defn of implication. We 
have then shown that both are cases are true, so we are done. 
