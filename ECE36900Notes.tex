...
\begin{table}[ht]
    ...
    \hline
    Universal generalization & ug & $P(x)$ & $(\exists)P(x)$\\
    \hline
...
any quantifiers.}
Let's see some of these rules in action by proving 
$P(x) \rightarrow (\forall y) (Q(x,y))$. 
\begin{align*}
    (\forall x)(\forall y)L(x,y) \rightarrow [(\exists x) H(x)] \quad &\text{(hypothesis)}\\
    P(x) \quad &\text{(hypothesis)} \\
    P(x) \rightarrow Q(x,y) \quad &\text{(1, ui)} \\
    Q(x,y) \quad &\text{(2, 3, mp)} \\
    (\forall y) Q(x,y) \quad &\text{(4, ug)} \\
    \hline
    \therefore P(x) \rightarrow (\forall y) (Q(x,y)) \notag
\end{align*}
