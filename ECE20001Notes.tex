...
yields $V_x = 10$ V, meaning $I_0 = 1$ A. Since the Norton current is the 
current through $a$ and $b$ when they are shorted, $I_N = 0.2I_0 = 0.2$ A. Likewise, since the 
Thevenin voltage is the voltage across $a$ and $b$, the Thevenin voltage is 10 V. 

Well and good, but what if we want to try source transforming? Let's do just that
