...
\begin{enumerate}
    ...
    \item Voltage in RC circuit: $v_c(\infty) +\left(v_c{t_0} - v_c(\infty)\right)e^{(\frac{-1}{RC})(t - t_0)}$
\end{enumerate}
...
\defn{First order circuit}{contain only one energy storage element 
...
and so on. If we have more than one energy-storing component then 
...
\begin{align*}
    V &= iR + V_c \\
    &= CR\frac{V_c}{dt} + V_c \\
    &\rightarrow \frac{-1}{RC}V_c + \frac{V}{RC} \\
    &= \frac{di}{dt}R+\frac{dV_c}{dt} \\
    &= \frac{di}{dt}R = \frac{i}{C} \\
    &\rightarrow \frac{-1}{RCi} + \frac{1}{R}\frac{V}{R} \\
    &\rightarrow v(t) = v(t_0)e^{\lambda (t-t_0)}
\end{align*}
Where $\lambda = -\frac{1}{RC}$ and $v(t)$ is the voltage (across the capacitor or source, it doesn't matter). 

% do a couple HW problems in here as examples
