...
Let's do an example to see Thevenin and Nortion's theorems in action. 
...
\end{figure}
Let's simplify 
this circuit a bit before trying to transform it. First, notice 
that we have $2V_x-30$ V across the $10 \Omega$ resistor. 
By Ohm's law, $I_0 = \frac{30-2V_x}{10}$. Looking at the right loop,
we have that the current flowing across the resistor is $0.2\frac{2V_x-30}{10}$. 
That means $V_x = 50 * 0.2\frac{2V_x-30}{10} = 10 \frac{2V_x-30}{10}$. Let's
organize our equations. 
\begin{align*}
    I_0 &= \frac{2V_x-30}{10} \\
    V_x &= 10 \frac{2V_x-30}{10}
\end{align*}
In this case, we don't even need linear algebra to solve. The second equation
yields $V_x = 30$ V, meaning $I_0 = 3$ A. Since the Norton current is the 
current through $a$ and $b$, $I_N = I_0 = 3$ A. Likewise, since the 
Thevenin voltage is the voltage across $a$ and $b$, the Thevenin voltage is 30 V. 
Vx = 10
vx and vth are in parallel so smae voltagedrop
To find Norton current, short across the terminals 
N = 0.2 I0

For question 8, you can transform the current and resistor in parallel to 
voltage and resistor in series. then sum around and all are 0
\end{document}
