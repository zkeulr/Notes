...
The values within the integral perform operations on the functions. That is, $f(t)$ becomes $f(\tau)$, while $g(t)$ becomes $g(-\tau)$ (changes handedness) and is moved along the $\tau$ axis from $-\infty$ to $\infty$. The convolution is the integral of the product of these two functions.\\
...
There is an important function within convolutions that we defined in a prior section. And it is important because it has the following properties:
\begin{equation*}
...
We will now look at the properties of the $\delta$ function stated previously. Since 
\end{document}
